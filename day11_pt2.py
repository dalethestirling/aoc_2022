...
with open("./day11_input.txt") as f:
    for line in f:
        if line.startswith('Monkey'):
            ...
            monkeys[monkey_id].items = [float(item) for item in line.strip().split(':')[1].strip().split(',')]
...
modulus = lcm(*[monkey.test for monkey in monkeys.values()])

while rounds < num_rounds:
    for monkey in monkeys:
        for _ in range(len(monkeys[monkey].items)):
            item = monkeys[monkey].items.pop()
            worry_adjusted = monkeys[monkey].operation(item)
            worry_adjusted %= modulus
